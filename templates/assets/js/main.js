document.addEventListener('DOMContentLoaded', function () {
    ...
    const addRuntime = () => {
        ...
        if ($runtimeCount) {
            const s1 = new Date(buildDate.replace(/-/g, "/")); // 转换为 Date 对象
            const s2 = new Date(); // 当前时间
            const days = s2.getTime() - s1.getTime(); // 毫秒差值
            const number_of_days = parseInt(days / (1000 * 60 * 60 * 24)); // 换算为天数
            $runtimeCount.innerText = number_of_days + "天"; // 写入页面
        }
    };
